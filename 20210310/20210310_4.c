...
void getShipmentInfo(double *priceShipment, double *priceTotalShipment){
    ...
    if (errno == 0){
        fprintf(fp, "getShipmentInfo() success\n");
    }
    else {
        fprintf(fp,"\n getShipmentInfo() has errno: %d : %s\n", errno, strerror(errno));
    }
}
...
void findBestShipmentOption(double *priceShipment, double *priceTotalShipment, int numShipment){
    ...
    if (errno == 0){
        fprintf(fp, "findBestShipmentOption() success\n");
    }
    else {
        fprintf(fp,"\n findBestShipmentOption() has errno: %d : %s\n", errno, strerror(errno));
    }
}
